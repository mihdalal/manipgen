...
from manipgen.real_world.utils.homography_utils import save_pointcloud
from manipgen.real_world.utils.geometry import TorchCuboids, TorchSpheres
...
SELF_COLLISION_SPHERES = [
    ...
    ("panda_hand", [0.0, 0.06, 0.12], 0.03),  # UMI gripper
    ("panda_hand", [0.0, -0.06, 0.12], 0.03),
    ("panda_hand", [0.0, 0.06, 0.16], 0.03),
    ("panda_hand", [0.0, -0.06, 0.16], 0.03),
    ("panda_hand", [0.0, 0.06, 0.20], 0.03),
    ("panda_hand", [0.0, -0.06, 0.20], 0.03),
]
...
SELF_COLLISION_UMI = [
    ("panda_hand", [0.0, 0.06, 0.15], [0.04, 0.04, 0.14]),
    ("panda_hand", [0.0, -0.06, 0.15], [0.04, 0.04, 0.14]),
]

# parameters for collision checking cuboids
CENTERS_C = [[0.0, 0.0, 0.14/2]]
SIZE_C = [[0.03, 0.16, 0.14]]
# parameters for collision checking spheres
CENTERS_S = []
RAIUS_S = []
...
class FrankaCollisionChecker:
    ...
    def torch_spheres_cuboids(self, configs: torch.Tensor):
        ...
        cuboid_dims = torch.Tensor(SIZE_C).to(self.device).unsqueeze(0).repeat(B, 1, 1)

        eef_quaternions_wxyz = conversions.rotation_matrix_to_quaternion(
            link_transforms[:, 8:9, :3, :3]
        )
        cuboid_quaternions_wxyz = eef_quaternions_wxyz.repeat(1, Mc, 1)
        ...
        torch_cuboids = TorchCuboids(
            ...
            cuboid_quaternions_wxyz.reshape(B, Mc, 4),
        )
    ...
    def check_scene_sdf_batch(
        ...
        if debug:
            ...
            spheres_pcd = torch_spheres.sample_surface(100).reshape(-1, 3).cpu().numpy()
            cuboids_pcd = torch_cuboids.sample_surface(100).reshape(-1, 3).cpu().numpy()
